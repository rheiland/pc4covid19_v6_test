...
# def update_this_leaf_cell_def_params(xmlpath, save_param_val, cell_def_name):
def update_this_leaf_cell_def_params(xmlpath, save_param_val, cd_uep):
#    for cell_def in immune_cell_defs:
    cell_def_name = cd_uep.attrib["name"]
    for cd in xml_flat_root.findall('cell_definitions//cell_definition'):  # find *this* cell_def in flattened XML
...
def recurse_node2(root, xmlpath, cd_uep):
    global save_param_val
    cell_def_name = cd_uep.attrib["name"]
    xmlpath = xmlpath + "//" + root.tag[root.tag.rfind('}')+1:]
    param_val = ''
    for child in root:
        ...
        recurse_node2(child,xmlpath,cd_uep)

    if len(list(root)) == 0:   # reached the "end"
        # print(xmlpath)
        print("recurse_node2: xmlpath = ",xmlpath,' = ',save_param_val)
        # recurse_node2: xmlpath =  //phenotype//secretion//substrate//uptake_rate  =  0.01
        if "secretion//substrate" in xmlpath:
            print("   found = secretion//substrate")
            # uep = xml_flat_root.find('cell_definitions//cell_definition'.attrib["name"]):  # find *this* cell_def in flattened XML
            uep = cd_uep.find('phenotype//secretion//substrate')
            print("   uep = ",uep)
            substrate_name = uep.attrib["name"]
            print("   name = ",substrate_name)


            cell_def_name
            # In [15]: uep = tree.find('cell_definitions//cell_definition[@name="lung epithelium"]//phenotype//secretion//substrate')
            xmlstr = 'cell_definitions//cell_definition[@name="' + cell_def_name + '"]//phenotype//secretion//substrate[@name="' + substrate_name + '"]' + xmlpath[xmlpath.rfind('//'):]
            print("   xmlstr = ",xmlstr)
            uep = xml_flat_root.find(xmlstr)
            # cd.find('.'+xmlpath).text = save_param_val
            uep.text = save_param_val
            # if uep.attrib["name"] in leaf_name:  # rwh: flatten3.py
            # In [15]: uep = tree.find('cell_definitions//cell_definition[@name="lung epithelium"]//phenotype//secretion//substrate')
            # In [16]: uep.attrib['name']
            # Out[16]: 'pro-pyroptosis cytokine'
            save_param_val = None

    # if cd.attrib["name"] in leaf_immune_cell_defs:
    # if cd.attrib["name"] in leaf_name:  # rwh: flatten3.py
        # print("recurse_node2: calling update_this_leaf_cell_def_params")
        # update_this_immune_cell_def_params(xmlpath, save_param_val, cell_def_name)
        # update_this_leaf_cell_def_params(xmlpath, save_param_val, cell_def_name)
        else:
            update_this_leaf_cell_def_params(xmlpath, save_param_val, cd_uep)
            save_param_val = None
...
print("leaf_immune_cell_defs= ",leaf_immune_cell_defs)
#leaf_name =  ['lung epithelium', 'CD8 Tcell', 'macrophage', 'neutrophil', 'DC', 'CD4 Tcell', 'fibroblast', 'residual']
#leaf_immune_cell_defs=  ['CD8 Tcell', 'macrophage', 'neutrophil', 'DC', 'CD4 Tcell', 'fibroblast']
# sys.exit()
for cd in xml_orig.findall('cell_definitions//cell_definition'):   # in the original, hierarchical config file
    idx += 1
    # if cd.attrib["name"] in leaf_immune_cell_defs:
    if cd.attrib["name"] in leaf_name:  # rwh: edit for flatten3.py
    # if cd.attrib["name"] in ['lung epithelium']:  # rwh: debug flatten3.py
        uep = cd
        print("\n---------------- processing ",cd.attrib["name"])   # 2  (0=default, 1=lung epithelium, etc)
        # immune_uep = root.find('.//cell_definitions')
        for child in cd:  # for each child of the cell_def
            print("------- calling recurse_node2 on child=",child)
            # recurse_node2(child,"",cd.attrib["name"])
            recurse_node2(child,"",cd)  # recurse until you hit the "end"; tricky for <secretion><substrate>s
