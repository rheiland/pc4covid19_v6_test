...
#s1 = tree1.find('secretion')
#s2 = tree2.find('secretion')

def update_all_substrate_params(xmlpath, save_param_val, substrate_name):
        # for elm in xml_root1.findall('secretion//substrate'):
        for elm in xml_root1.findall('secretion'):
                print('-- update ',elm, ', xmlpath=',xmlpath, " = ",save_param_val)
                if "secretion//substrate" in xmlpath:
                    print("=======  need special handling of secretion//substrate  for ", substrate_name)
                    if "uptake_rate" in xmlpath[-13:]:
                        print("\n=======     handle uptake_rate")
                        parent_elm = elm.find('.' + xmlpath[:-13])
                        print("=======     xmlpath[:-13]",xmlpath[:-13] )
                        print("=======     this substrate(parent) name= ",parent_elm.attrib["name"] )
                        print("=======     substrate_name as param= ",substrate_name )
                        if (substrate_name == parent_elm.attrib["name"]):
                            print(" WEEEEEEEEEEEEE!!! match found")
                    parent_elm = elm.find('.' + xmlpath[:-13])
                    print('-- parent ',parent_elm)
                    # print("   parent=", cd.find('.'+xmlpath).getparent() )
                elm.find('.'+xmlpath).text = save_param_val
...
def recurse_node(root,xmlpath,substrate_name):
    ...
    if (root.tag == "substrate") and (len(root.attrib) > 0):   # don't want substrate for chemotaxis, just secretion
        print("========>> found substrate (for secretion), attrib=",root.attrib)
        substrate_name = root.attrib["name"]
        print("========>> substrate_name =",substrate_name ,"\n")
    for child in root:
