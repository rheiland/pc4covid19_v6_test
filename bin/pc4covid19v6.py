...
def write_config_file(name):
    ...
    # print("pc4covid19_v5.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)


# callback from write_config_button
# def write_config_file_cb(b):
#     path_to_share = os.path.join('~', '.local','share','pc4covid19_v5')
...
def get_config_files():
    ...
    cf = {"baseline": baseline_xml_full_filename, "low MOI":low_moi_xml_full_filename, "high MOI":high_moi_xml_full_filename, "immune":immune_no_local_xml_full_filename }
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/pc4covid19_v6")  # does Windows like this?
    else:
        # local cache
        try:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB pc4covid19_v6' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc4covid19_v6-r7 config.xml")   # "-r7" suffix??
        else:
...
# tool_title = widgets.Label(r'\(\textbf{pc4covid19_v5}\)')
tool_title = widgets.Label(r'\(\textbf{PhysiCell model for SARS-CoV-2 (v6)}\)')
if nanoHUB_flag or hublib_flag:
